...
if(file.exists("../dbf/regrasV4_aptidao_regras.rds")){
    ...
    if(!file.exists("../dbf/regrasV4_aptidao_regras_modelo_multinom.rds")){
        modelo <- multinom(aptidao ~ ., data = conjunto_treino)
        saveRDS(modelo, "../dbf/regrasV4_aptidao_regras_modelo_multinom.rds")
    }else{
        modelo <- readRDS("../dbf/regrasV4_aptidao_regras_modelo_multinom.rds")
    }
    previsoes <- predict(modelo, newdata = conjunto_teste, type = "class")
    datasetfull[-indice_treino, "aptidao"] <- previsoes

    if(!file.exists("../dbf/regrasV4_aptidao_regras_modelo_multinom_aplicado.rds")){
        saveRDS(datasetfull, "../dbf/regrasV4_aptidao_regras_modelo_multinom_aplicado.rds")
    }
